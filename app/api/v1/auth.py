...
@router.post(
    path='/signup',
    response_model=schemas_auth.UserSighup,
...
async def signup(
    ...
    try:
        user, tokens = await auth_service.register(user_data=user_data)
    ...
    user_read = schemas_user.UserRead.model_validate(user)
    return schemas_auth.UserSighup(
