...
async def test_get_user_by_id(fake_user, user_service):
    user_service.user_repo.get_user_by_id.return_value = fake_user
    user = await user_service.get_user_by_id(user_id=fake_user.id)
    assert user is not None
    assert user.id == fake_user.id


async def test_get_user_by_id_not_found(fake_uuid, user_service):
...
async def test_update_user_success(
    ...
    user_service.user_repo.save_user.return_value = User(
